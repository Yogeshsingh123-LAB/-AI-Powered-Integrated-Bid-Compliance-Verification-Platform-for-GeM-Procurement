...
export function useChatWindow() {
  const [rect, setRect] = useState(() => {
    try { const saved = JSON.parse(localStorage.getItem(KEY)); if (saved && ["x", "y", "width", "height"].every(k => Number.isFinite(saved[k]))) return fitWindow(saved, viewport()); } catch { /* Storage is optional. */ }
    return defaults();
  });
  ...
  useEffect(() => {
    const update = () => { setView(viewport()); setRect(r => fitWindow(r, viewport())); };
  ...
  const displayed = mobile ? { x: 8, y: expanded ? 8 : Math.max(8, view.height - 608), width: view.width - 16, height: expanded ? view.height - 16 : Math.min(600, view.height - 16) }
    : expanded ? { x: 8, y: 8, width: view.width - 16, height: view.height - 16 } : rect;
  function start(event, edge = "move") {
    if (mobile || expanded || event.button !== 0) return;
    event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId);
    drag.current = { id: event.pointerId, x: event.clientX, y: event.clientY, rect, edge };
  }
  function move(event) {
  ...
  function keyboard(event, edge = "move") {
    ...
    setRect(r => edge === "move" ? fitWindow({ ...r, x: r.x + dx, y: r.y + dy }, viewport()) : resizeWindow(r, edge, dx, dy, viewport()));
  }
