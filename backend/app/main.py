...
async def lifespan(app: FastAPI):
    try:
        ...
        import logging
        logging.getLogger(__name__).warning(f"Database initialization warning: {exc}")
    try:
        yield
    finally:
        try:
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    return JSONResponse(
        status_code=422,
        content={"success": False, "detail": "Validation error: " + str(exc.errors())}
    )

@app.exception_handler(Exception)
async def global_exception_handler(request: Request, exc: Exception):
    import logging
    logging.getLogger("app.main").error(f"Unhandled server error: {exc}", exc_info=True)
    return JSONResponse(
        status_code=500,
        content={"success": False, "detail": f"An unexpected server error occurred: {str(exc)}"}
    )
